 ...
 */
// Функція для створення та додавання DOM-елемента до контейнера
...
function createDomElement(tagName, textContent, container) {
    ...
    // Повернення створеного елемента
    return newElement;
}
...
function setUserInfoCookie(key, value) {
    // Кодування значення параметра
    const encodedValue = encodeURIComponent(`${key}=${value}`);

    // Створення дати закінчення терміну дії cookie (через 10 секунд)
